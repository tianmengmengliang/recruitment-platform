      ...
      <div style={{ maxHeight: 500, overflow: 'auto' }}>
        {res?.msg?.split('\n')?.map((item: any, i: number) => (
          <div key={i} style={{ textAlign: 'left' }}>
            <pre style={{ marginBottom: '0px', overflow: 'unset' }}>{item}</pre>
          </div>
        ))}
      </div>
    </>
...
export const timeFile = (data: string) => {
  ...
  if (Array.isArray(list)) {
    ...
    dataList?.forEach((item: any, i: any) => {
      ...
      const row = item.split(',').map((key: any) => Number(key));
      algorithmItem = {
        date: i + 1,
        name: 'Algorithm time',
...
export const statusEnum = {
  ...
  active: { text: 'active', status: 'error' },
};
...
export const handleError = (response: any) => {
  ...
  if (response && response.status === 500) {
    ...
    const meg = response.statusText
      ? `${arr[arr.length - 1].split('?')[0]} ${response.statusText}`
      : '查询数据失败！';
    message.error(meg, 3);
  } else if (!response) {
    notification.error({
...
export const saveToFile = (data: string, name: string) => {
  ...
  const blob = new Blob([data]); // JSON.stringify(data)
